...
export default function PatientProfile() {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          setError('Profile not found. Please create your patient profile first.');
        } else {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axiosInstance.put('/api/patients/me', {
        userId: null,
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">My Patient Profile</h1>
            <p className="text-sm text-gray-500 mt-1">Keep your personal and emergency details up to date.</p>
          </div>
          <Link
            ...
            <>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4" noValidate>
                ...
                <Field
                  ...
                  input={<input id="bloodGroup" className={inputClass(errors.bloodGroup)} {...register('bloodGroup')} />}
                />
